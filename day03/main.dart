  ...
  for (int i = 0; i < rucksacks.length; i += 3) {
    ...
    for (int j = 0; j < 3; j++) {
      for (int k = 0; k < rucksacks[i + j].length; k++) {
        if ((H[rucksacks[i + j][k]] != null && H[rucksacks[i + j][k]] == j) ||
            (H[rucksacks[i + j][k]] == null && j == 0)) {
          H[rucksacks[i + j][k]] = j + 1;
        }
      }
    }
    sum += M[H.entries
        .singleWhere((MapEntry<String, int> item) => item.value == 3)
        .key]!;
  }
  print(sum);
}

void partTwoSimpler(final List<String> rucksacks) {
  int sum = 0;
  for (int i = 0; i < rucksacks.length; i += 3) {
    for (int j = 0; j < rucksacks[i].length; j++) {
      if (rucksacks[i + 1].contains(RegExp(rucksacks[i][j])) &&
          rucksacks[i + 2].contains(RegExp(rucksacks[i][j]))) {
        sum += M[rucksacks[i][j]]!;
        break;
      }
    }
